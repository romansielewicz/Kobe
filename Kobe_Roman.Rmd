...
court <- rasterGrob(readJPEG(getURLContent(courtImg.URL)),
           width=unit(1,"npc"), height=unit(1,"npc"))
heatmap2001 <- ggplot(subset(map.zone.range, season == "2000-01"), aes(x=loc_x, y=loc_y)) +
      annotation_custom(court, -250, 250, -50, 420) +
      geom_point(aes(colour = percent, shape = shot_made_flag)) +
      xlim(-250, 250) +
      ylim(-50, 420) +scale_color_gradientn(colours = rainbow(8))

heatmap.animation <- ggplot(map.zone.range, aes(x=loc_x, y=loc_y)) +
      annotation_custom(court, -250, 250, -50, 420) +
      geom_point(aes(colour = percent, shape = shot_made_flag, frame = season)) +
      xlim(-250, 250) +
      ylim(-50, 420) + scale_color_gradientn(colours = rainbow(8))
